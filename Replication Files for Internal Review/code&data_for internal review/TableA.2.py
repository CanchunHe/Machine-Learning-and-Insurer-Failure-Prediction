...
def FFN(test_data,neurons,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        with open(r'models_final_TableA.2\FFN'+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','rb') as f:
...
def logistic(data,datatype,year,predict_label):
    with open(r'models_final_TableA.2\logit'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','rb') as f:
        model = pickle.load(f)
    return model

def TreeModel(data,method,datatype,year,predict_label):
    if method=='RF':
        with open(r'models_final_TableA.2\RF'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
    elif method=='XGBoost':
        with open(r'models_final_TableA.2\XGBoost'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
    return select_model
...
def DFFN(test_data1,neurons1,test_data2,neurons2,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        with open(r'models_final_TableA.2\DFFN'+str(neurons1)+str(neurons2)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
...
def DFFN2(test_data1,neurons1,test_data2,neurons2,neurons,datatype,year,predict_label,ensemble=10):
    ...
    X_test2 = torch.FloatTensor(test_data2.iloc[:,3:-1].values)
    for i in range(ensemble):
        with open(r'models_final_TableA.2\DFFN'+str(neurons1)+str(neurons2)+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
...
def TFFN(test_data1,neurons1,test_data2,neurons2,test_data3,neurons3,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        with open(r'models_final_TableA.2\TFFN'+str(neurons1)+str(neurons2)+str(neurons3)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
...
def TFFN2(test_data1,neurons1,test_data2,neurons2,test_data3,neurons3,neurons,datatype,year,predict_label,ensemble=10):
    ...
    X_test3 = torch.FloatTensor(test_data3.iloc[:,3:-1].values)
    for i in range(ensemble):
        with open(r'models_final_TableA.2\TFFN'+str(neurons1)+str(neurons2)+str(neurons3)+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','rb') as f:
            select_model = pickle.load(f)
...
if __name__ == "__main__":  
    ...
    for datatype in ['firm', 'firm+yield', 'firm+macro', 'firm+macro+yield']:
        ...
        Table4 = pd.concat([Table4, auc])
    
        with pd.ExcelWriter('tables and figures/TableA.2.xlsx') as writer:
            Table4.to_excel(writer,sheet_name='TableA.2')