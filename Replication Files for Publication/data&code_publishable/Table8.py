...
if __name__ == "__main__":  
   ...
   #load datas
    ...
    fixed_type_ii_values = np.arange(0.1, 0.35, 0.1)  # from 10% to 30%
    for fixed_ii in fixed_type_ii_values:
       for datatype in ['firm', 'firm+yield', 'firm+macro', 'firm+macro+yield']:
           ...
           type_i_error_rates=pd.DataFrame(index=[datatype])
           for method in results.columns[3:]:
               fpr,tpr,thresholds = metrics.roc_curve(results[predict_label],results[method])
               fnr=1-tpr #type ii error rate    
               idx = (np.abs(fnr - fixed_ii)).argmin() 
               corresponding_fpr = fpr[idx]  # Type I error rate (FPR)
               type_i_error_rates[method] = [corresponding_fpr]  
               auc[method] = metrics.auc(fpr,tpr)
           Table8 = pd.concat([Table8, type_i_error_rates])
     
    #save results
    with pd.ExcelWriter('tables and figures/Table8.xlsx') as writer:
        Table8.to_excel(writer,sheet_name='Table8')