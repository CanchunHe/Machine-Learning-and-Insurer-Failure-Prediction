...
#%%
##training part: using the folder "models_final_TableA.2"
...
def FFN(train_data,neurons,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        ...
        for k in range(1000):       
            ...
            if j> 50:
                break
        select_model.eval()
        with open(r'models_final_TableA.2\FFN'+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','wb') as f:
...
def logistic(data,datatype,year,predict_label):
    ...
    with open(r'models_final_TableA.2\logit'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','wb') as f:
        pickle.dump(model,f)
...
def TreeModel(data,method,datatype,year,predict_label):
    ...
    if method=='RF':
        ...
        with open(r'models_final_TableA.2\RF'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)
            ...
            for max_depth in range(3,7,1):
                for reg_lambda in [1e-3,1e-2,1e-1,1,10]:
                    ...
                    if new_error < error:
                        error = new_error
                        select_model = copy.deepcopy(model)
        with open(r'models_final_TableA.2\XGBoost'+'_'+datatype+'_'+str(year)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)
...
def DFFN(train_data1,neurons1,train_data2,neurons2,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        ...
        select_model.eval()
        with open(r'models_final_TableA.2\DFFN'+str(neurons1)+str(neurons2)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)
...
def DFFN2(train_data1,neurons1,train_data2,neurons2,neurons,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        ...
        select_model.eval()
        with open(r'models_final_TableA.2\DFFN'+str(neurons1)+str(neurons2)+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)
...
def TFFN(train_data1,neurons1,train_data2,neurons2,train_data3,neurons3,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        ...
        select_model.eval()
        with open(r'models_final_TableA.2\TFFN'+str(neurons1)+str(neurons2)+str(neurons3)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)
...
def TFFN2(train_data1,neurons1,train_data2,neurons2,train_data3,neurons3,neurons,datatype,year,predict_label,ensemble=10):
    ...
    for i in range(ensemble):
        ...
        select_model.eval()
        with open(r'models_final_TableA.2\TFFN'+str(neurons1)+str(neurons2)+str(neurons3)+str(neurons)+'_'+datatype+'_'+str(year)+'_'+str(i)+'_'+predict_label+'.pk','wb') as f:
            pickle.dump(select_model,f)


if __name__ == "__main__":  
    if 'models_final_TableA.2' not in os.listdir():
        os.mkdir('models_final_TableA.2')
# TableA.2
    #load datas
